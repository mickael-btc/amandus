    ...
    db.users.collection("users").findOne(
      ...
      (err, userData) => {
        ...
        settingsHandler
          ...
          .then((settings) => {
            let overQuota = Number(userData.quota || (config.maxStorage ? config.maxStorage * 1024 * 1024 : settings["const:max:storage"])) - userData.storageUsed <= 0;
              ...
              validateFromAddress(tools.normalizeAddress(envelope.from.address), (err, address) => {
                ...
                validateFromAddress(tools.normalizeAddress(options.from.address), (err, address) => {
                  ...
                  let addToDeliveryQueue = (next) => {
                    ...
                    messageHandler.counters.ttlcounter("wdr:" + userData._id.toString(), compiledEnvelope.to.length, maxRecipients, false, (err, result) => {
                      if (err) {
                        err.responseCode = 500;
                        err.code = "InternalDatabaseError";
                        return callback(err);
                  ...
                  addToDeliveryQueue((err, outbound) => {
                    ...
                    messageHandler.encryptMessage(userData.encryptMessages ? userData.pubKey : false, { chunks: collector.chunks, chunklen: collector.chunklen }, (err, encrypted) => {
                      let raw = false;
                      if (!err && encrypted) {
