      ...
      let processNext = () => {
        ...
        cursor.next((err, messageData) => {
          ...
          db.database.collection("archived").deleteOne({ _id: messageData._id }, (err) => {
            if (err) {
              //failed to delete
              log.error("GC", "Failed to delete archived message user=%s mailbox=%s uid=%ss id=%s. %s", messageData.user, messageData.mailbox, messageData.uid, messageData._id, err.message);
              return cursor.close(() => done(err));
            }

            log.verbose("GC", "Deleted archived message user=%s mailbox=%s uid=%s id=%s", messageData.user, messageData.mailbox, messageData.uid, messageData._id);
